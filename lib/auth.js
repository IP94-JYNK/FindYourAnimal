...
module.exports = () => {
  ...
  const getInterlocutors = async email => {
    const dialogs = await getDialogsByEmail(email);
    let interlocutors = [];

    const interlocutorsPromises = dialogs.map(async dialog => {
      const interlocutorEmail = dialog.user1 === email ? dialog.user2 : dialog.user1;
      const interlocutor = await getUserByEmail(interlocutorEmail);
      const interlocutorName = await getUserName(interlocutor.id);
      return interlocutorName;
    });

    for await (const interlocutor of interlocutorsPromises) {
  ...
  const createDialog = async (user1, user2) => {
    db.insert('Dialog', { user1, user2 });
  };

  const createMessage = async (user1, dialogId, content) => {
    db.insert('Message', { user1, dialogId, content });
  };
